  ...
  const getData = async () => {
    try {
      const response = await fetch(
        "https://www.neomiannay.fr/php-laylow/album.php?id=" + props.route.params.id
      );
    ...
    >
      {isLoading ? (
        ...
        <View style={{flex: 1, flexDirection: 'column'}}>
          <Ionicons style={Padding(48, "y")} name={"chevron-back"} size={32} color={"#ffffff"} onPress={() => props.navigation.goBack()}/>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={styles.wrapper}>
                ...
                <Text style={[styles.text, {textAlign: 'right'}]}>{item.duree}</Text>
              </View>
            )}
          />

          <Text style={[TextStyles.bottomText, {paddingLeft: 16}]}>{data.date}</Text>
          <Text style={[TextStyles.bottomText, {paddingLeft: 16}]}>{data.pistes} morceaux, {data.duree}</Text>
          <Text style={[TextStyles.bottomText, {paddingLeft: 16, marginBottom: 16}]}>{data.label}</Text>
          <Pressable
            style={TextStyles.button}
            onPress={() => Linking.openURL(data.spotify)}
          >
            <Text style={[[TextStyles.buttonContent], {color: "#1DB954"}]}>Écouter sur Spotify</Text>
          ...
          >
            <Text style={[[TextStyles.buttonContent], {color: "#f94c57"}]}>Écouter sur Apple Music</Text>
          ...
          >
            <Text style={[[TextStyles.buttonContent], {color: "#00C7F2"}]}>Écouter sur Deezer</Text>
